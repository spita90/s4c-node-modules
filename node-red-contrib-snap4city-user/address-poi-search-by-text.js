...
module.exports = function (RED) {
    ...
    function AddressPoiSearchByTextUsr(config) {
        ...
        node.on('input', function (msg) {
            ...
            console.log(encodeURI(uri + "?search=" + search + "&maxResults=" + maxResults + "&format=json" + (typeof uid != "undefined" && uid != "" ? "&uid=" + uid : "") + "&appID=iotapp"));
            xmlHttp.open("GET", encodeURI(uri + "?search=" + search + "&maxResults=" + maxResults + "&format=json" + (typeof uid != "undefined" && uid != "" ? "&uid=" + uid : "") + "&appID=iotapp"), true); // false for synchronous request
    ...
    RED.nodes.registerType("address-poi-search-by-text-usr", AddressPoiSearchByTextUsr);
}