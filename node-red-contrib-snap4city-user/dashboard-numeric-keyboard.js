...
module.exports = function (RED) {
    ...
    function NumericKeyboard(config) {
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("numeric-keyboard node " + node.name + " is being removed from flow");

                node.deleteEmitter();
            } else {
                // Riavvio nodo
                util.log("numeric-keyboard node " + node.name + " is being rebooted");
        ...
        node.wsCloseCallback = function (e) {
            ...
            util.log("numeric-keyboard closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("numeric-keyboard wsServerRetryActive: " + wsServerRetryActive);
            //util.log("numeric-keyboard node.notRestart: " + node.notRestart);
        ...
        node.wsInit = function (e) {
            ...
            try {
                ...
                if (node.ws == null) {
                    ...
                    util.log("numeric-keyboard node " + node.name + " already open WebSocket");
                }
            } catch (e) {
