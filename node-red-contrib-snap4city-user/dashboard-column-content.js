...
module.exports = function (RED) {
    ...
    function ColumnContentNode(config) {
        ...
        node.on('input', function (msg) {
            util.log("Flow input received for column-content node " + node.name + ": " + JSON.stringify(msg));
            ...
            if ((new Date().getTime() - node.wsStart) > parseInt(RED.settings.wsReconnectTimeout ? RED.settings.wsReconnectTimeout : 1200) * 1000) {
                ...
                util.log("column-content node " + node.name + " is reconnetting to open WebSocket");
                timeout = 1000;
            }
            ...
            setTimeout(function () {
                try {
                    ...
                    util.log("Error sending data to WebSocket for column-content node " + node.name + ": " + e);
                }
            }, timeout);
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("column-content node " + node.name + " is being removed from flow");
                node.deleteMetric();
            } else {
                // Riavvio nodo
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardId != null && node.dashboardId != "") {
                ...
                var payload = {
                    ...
                    flowId: node.z,
                    flowName: node.flowName,
                    widgetType: "widgetColumnContent",
                ...
                util.log("column-content node " + node.name + " IS GOING TO CONNECT WS");
            ...
            switch (response.msgType) {
                case "AddEditMetric":
                    if (response.result === "Ok") {
                        ...
                        util.log("WebSocket server correctly added/edited metric type for column-content node " + node.name + ": " + response.result);
                        ...
                        util.log("WebSocket server could not add/edit metric type for column-content node " + node.name + ": " + response.result);
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: response.error
                ...
                case "DelMetric":
                    if (response.result === "Ok") {
                        util.log("WebSocket server correctly deleted metric type for column-content node " + node.name + ": " + response.result);
                    } else {
                        //TBD - CASI NEGATIVI DA FARE
        ...
        node.wsCloseCallback = function (e) {
            util.log("column-content node " + node.name + " closed WebSocket");
            util.log("column-content closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("column-content wsServerRetryActive: " + wsServerRetryActive);
            //util.log("column-content node.notRestart: " + node.notRestart);
        ...
        node.wsErrorCallback = function (e) {
            util.log("column-content node " + node.name + " got WebSocket error: " + e);
        };

        node.deleteMetric = function () {
            util.log("Deleting metric via webSocket for column-content node " + node.name);
            var newMsg = {
                msgType: "DelMetric",
                nodeId: node.id,
                metricName: encodeURIComponent(node.metricName),
            ...
            try {
                ...
                util.log("Error deleting metric via webSocket for column-content node " + node.name + ": " + e);
            }
        };
        ...
        node.closedDoneCallback = function () {
            util.log("column-content node " + node.name + " has been closed");
        };

        node.wsInit = function (e) {
            util.log("column-content node " + node.name + " is trying to open WebSocket");
            try {
                node.status({
                    fill: "yellow",
                    shape: "dot",
        ...
        try {
            ...
            util.log("column-content node " + node.name + " got main exception connecting to WebSocket");
        }

    }

    RED.nodes.registerType("column-content", ColumnContentNode);
