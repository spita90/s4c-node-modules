...
module.exports = function (RED) {
    ...
    function DashboardTimeTrendNode(config) {
        ...
        node.on('input', function (msg) {
            util.log("Flow input received for dashboard-time-trend node " + node.name + ": " + JSON.stringify(msg));
            ...
            if ((new Date().getTime() - node.wsStart) > parseInt(RED.settings.wsReconnectTimeout ? RED.settings.wsReconnectTimeout : 1200) * 1000) {
                ...
                util.log("dashboard-time-trend node " + node.name + " is reconnetting to open WebSocket");
                timeout = 1000;
            }
            ...
            setTimeout(function () {
                try {
                    ...
                    util.log("Error sending data to WebSocket for dashboard-time-trend node " + node.name + ": " + e);
                }
            }, timeout);
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("dashboard-time-trend node " + node.name + " is being removed from flow");

                node.deleteMetric();
            } else {
                // Riavvio nodo
                util.log("dashboard-time-trend node " + node.name + " is being rebooted");
                node.notRestart = true;
                node.ws.terminate();
            }
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardId != null && node.dashboardId != "") {
                ...
                util.log("dashboard-time-trend node " + node.name + " IS GOING TO CONNECT WS");
            ...
            switch (response.msgType) {
                case "AddEditMetric":
                    if (response.result === "Ok") {
                        ...
                        util.log("WebSocket server correctly added/edited metric type for dashboard-time-trend node " + node.name + ": " + response.result);
                        ...
                        util.log("WebSocket server could not add/edit metric type for dashboard-time-trend node " + node.name + ": " + response.result);
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: response.error
                ...
                case "DelMetric":
                    if (response.result === "Ok") {
                        util.log("WebSocket server correctly deleted metric type for dashboard-time-trend node " + node.name + ": " + response.result);
                    } else {
                        //TBD - CASI NEGATIVI DA FARE
                        util.log("WebSocket server could not delete metric type for dashboard-time-trend node " + node.name + ": " + response.result);
                    }
                    util.log("Closing webSocket server for dashboard-time-trend node " + node.name);
                    node.notRestart = true;
                    node.ws.terminate();
                    break;
        ...
        node.wsCloseCallback = function (e) {
            util.log("dashboard-time-trend node " + node.name + " closed WebSocket");
            util.log("dashboard-time-trend closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("dashboard-time-trend wsServerRetryActive: " + wsServerRetryActive);
            //util.log("dashboard-time-trend node.notRestart: " + node.notRestart);
            if (wsServerRetryActive === 'yes' && !node.notRestart) {
                util.log("dashboard-time-trend node " + node.name + " will try to reconnect to WebSocket in " + parseInt(wsServerRetryTime) + "s");
                if (!node.intervalID) {
        ...
        node.wsErrorCallback = function (e) {
            util.log("dashboard-time-trend node " + node.name + " got WebSocket error: " + e);
        };

        node.deleteMetric = function () {
            util.log("Deleting metric via webSocket for dashboard-time-trend node " + node.name);
            var newMsg = {
                msgType: "DelMetric",
                nodeId: node.id,
                metricName: encodeURIComponent(node.metricName),
            ...
            try {
                ...
                util.log("Error deleting metric via webSocket for dashboard-time-trend node " + node.name + ": " + e);
            }
        };
        ...
        node.closedDoneCallback = function () {
            util.log("dashboard-time-trend node " + node.name + " has been closed");
        };

        node.wsInit = function (e) {
            util.log("dashboard-time-trend node " + node.name + " is trying to open WebSocket");
            try {
                node.status({
                    fill: "yellow",
                    shape: "dot",
                ...
                if (node.ws == null) {
                    ...
                    util.log("dashboard-time-trend node " + node.name + " already open WebSocket");
                }
            } catch (e) {
                util.log("dashboard-time-trend node " + node.name + " could not open WebSocket");
                node.status({
                    fill: "red",
                    shape: "ring",
        ...
        try {
            ...
            util.log("dashboard-time-trend node " + node.name + " got main exception connecting to WebSocket");
        }

    }

    RED.nodes.registerType("dashboard-time-trend", DashboardTimeTrendNode);
