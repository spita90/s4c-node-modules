...
module.exports = function (RED) {
    ...
    function Dimmer(config) {
        ...
        node.on('input', function (msg) {
            util.log("Flow input received for dimmer node " + node.name + ": " + JSON.stringify(msg));
            ...
            if ((new Date().getTime() - node.wsStart) > parseInt(RED.settings.wsReconnectTimeout ? RED.settings.wsReconnectTimeout : 1200) * 1000) {
                ...
                util.log("dimmer node " + node.name + " is reconnetting to open WebSocket");
                timeout = 1000;
            }
            node.wsStart = new Date().getTime();

            if (typeof node.widgetUniqueName != "undefined" && node.widgetUniqueName != "") {
                if (!Number.isNaN(parseFloat(msg.payload))) {
                    if (parseFloat(msg.payload) != node.lastValue) {
                        ...
                        setTimeout(function () {
                            try {
                                util.log("newMetricData: " + JSON.stringify(newMetricData));
                                node.ws.send(JSON.stringify(newMetricData));
                            } catch (e) {
                                util.log("Error sending data to WebSocket for dimmer node " + node.name + ": " + JSON.stringify(e));
                            }
                        }, timeout);
                    ...
                    node.error("You must insert in input a number");
                }
            } else {
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("dimmer node " + node.name + " is being removed from flow");

                node.deleteEmitter();
            } else {
                // Riavvio nodo
                util.log("dimmer node " + node.name + " is being rebooted");
                node.notRestart = true;
                node.ws.terminate();
            }
        ...
        node.wsOpenCallback = function () {
            if (node.dashboardId != null && node.dashboardId != "") {
                ...
                util.log("dimmer node " + node.name + " IS GOING TO CONNECT WS");
            ...
            switch (response.msgType) {
                case "AddEmitter":
                    if (response.result === "Ok") {
                        ...
                        if (!Number.isNaN(parseFloat(response.lastValue))) {
                            node.lastValue = parseFloat(response.lastValue);
                        }

                        util.log("WebSocket server correctly added/edited emitter type for dimmer node " + node.name + ": " + response.result);
                        node.status({
                            fill: "green",
                            shape: "dot",
                        ...
                        util.log("WebSocket server could not add/edit emitter type for dimmer node " + node.name + ": " + response.result);
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: response.error
                ...
                case "DelEmitter":
                    if (response.result === "Ok") {
                        util.log("WebSocket server correctly deleted emitter type for dimmer node " + node.name + ": " + response.result);
                    } else {
                        //TBD - CASI NEGATIVI DA FARE
                        util.log("WebSocket server could not delete emitter type for dimmer node " + node.name + ": " + response.result);
                    }
                    util.log("Closing webSocket server for dimmer node " + node.name);
                    node.notRestart = true;
                    node.ws.terminate();
                    break;
                case "DataToEmitter":
                    if (response.newValue != "dashboardDeleted") {
                        ...
                        if (!Number.isNaN(parseFloat(response.newValue))) {
                            node.lastValue = parseFloat(response.newValue);
                            var msg = {
        ...
        node.wsCloseCallback = function (e) {
            util.log("dimmer node " + node.name + " closed WebSocket");
            util.log("dimmer closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("dimmer wsServerRetryActive: " + wsServerRetryActive);
            //util.log("dimmer node.notRestart: " + node.notRestart);
            if (wsServerRetryActive === 'yes' && !node.notRestart) {
                util.log("dimmer node " + node.name + " will try to reconnect to WebSocket in " + parseInt(wsServerRetryTime) + "s");
                if (!node.intervalID) {
        ...
        node.wsErrorCallback = function (e) {
            util.log("dimmer node " + node.name + " got WebSocket error: " + e);
        };

        node.deleteEmitter = function () {
            util.log("Deleting emitter via webSocket for dimmer node " + node.name);
            var newMsg = {
                msgType: "DelEmitter",
                nodeId: node.id,
                user: node.username,
            ...
            try {
                ...
                util.log("Error deleting emitter via webSocket for dimmer node " + node.name + ": " + e);
            }
        };
        ...
        node.closedDoneCallback = function () {
            util.log("dimmer node " + node.name + " has been closed");
        };

        node.wsInit = function (e) {
            util.log("dimmer node " + node.name + " is trying to open WebSocket");
            try {
                node.status({
                    fill: "yellow",
                    shape: "dot",
                ...
                if (node.ws == null) {
                    ...
                    util.log("dimmer node " + node.name + " already open WebSocket");
                }
            } catch (e) {
                util.log("dimmer node " + node.name + " could not open WebSocket");
                node.status({
                    fill: "red",
                    shape: "ring",
        ...
        try {
            ...
            util.log("dimmer node " + node.name + " got main exception connecting to WebSocket");
        }
