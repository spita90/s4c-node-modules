...
module.exports = function (RED) {
    ...
    function GaugeChartNode(config) {
            ...
            switch (response.msgType) {
                case "AddEditMetric":
                    if (response.result === "Ok") {
                        ...
                        util.log("WebSocket server could not add/edit metric type for gauge-chart node " + node.name + ": " + response.result);
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: response.error
        ...
        node.wsCloseCallback = function (e) {
            ...
            util.log("gauge-chart closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("gauge-chart wsServerRetryActive: " + wsServerRetryActive);
            //util.log("gauge-chart node.notRestart: " + node.notRestart);
