...
module.exports = function (RED) {
    ...
    function ImpulseButton(config) {
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("impulse-button node " + node.name + " is being removed from flow");

                node.deleteEmitter();
            } else {
                // Riavvio nodo
                util.log("impulse-button node " + node.name + " is being rebooted");
        ...
        node.wsCloseCallback = function (e) {
            ...
            util.log("impulse-button closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("impulse-button wsServerRetryActive: " + wsServerRetryActive);
            //util.log("impulse-button node.notRestart: " + node.notRestart);
        ...
        node.wsInit = function (e) {
            ...
            try {
                ...
                if (node.ws == null) {
                    ...
                    util.log("impulse-button node " + node.name + " already open WebSocket");
                }
            } catch (e) {
