...
module.exports = function (RED) {
    ...
    function SwitchButton(config) {
        ...
        node.on('input', function (msg) {
            util.log("Flow input received for switch-button node " + node.name + ": " + JSON.stringify(msg));
            ...
            if ((new Date().getTime() - node.wsStart) > parseInt(RED.settings.wsReconnectTimeout ? RED.settings.wsReconnectTimeout : 1200) * 1000) {
                ...
                util.log("switch-button node " + node.name + " is reconnetting to open WebSocket");
                timeout = 1000;
            }
                    ...
                    if (node.lastValue != msg.payload) {
                        ...
                        setTimeout(function () {
                            try {
                                ...
                                util.log("Error sending data to WebSocket for switch-button node " + node.name + ": " + JSON.stringify(e));
                            }
                        }, timeout);

                        s4cUtility.eventLog(RED, msg, newMetricData, config, "Node-Red", "Dashboard", wsServer, "TX");
                    } else {
                        node.error("The switch-button is already set to the value entered");
                    }
                } else {
                    node.error("You must insert in input 'On' or 'Off'");
                }
            } else {
                node.error("Maybe the switch-button is not created on dashboard");
            }
        });
        ...
        node.on('close', function (removed, closedDoneCallback) {
            if (removed) {
                // Cancellazione nodo
                util.log("switch-button node " + node.name + " is being removed from flow");

                node.deleteEmitter();
            } else {
                // Riavvio nodo
                util.log("switch-button node " + node.name + " is being rebooted");
                node.notRestart = true;
                node.ws.terminate();
            }
        ...
        node.wsCloseCallback = function (e) {
            ...
            util.log("switch-button closed reason " + e);
            if (!(node.dashboardId != null && node.dashboardId != "")) {
                node.status({
                    fill: "red",
                    shape: "dot",
            ...
            //util.log("switch-button wsServerRetryActive: " + wsServerRetryActive);
            //util.log("switch-button node.notRestart: " + node.notRestart);
        ...
        node.wsInit = function (e) {
            ...
            try {
                ...
                if (node.ws == null) {
                    ...
                    util.log("switch-button node " + node.name + " already open WebSocket");
                }
            } catch (e) {
